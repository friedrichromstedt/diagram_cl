...
# $Last changed: 2009 Feb 23$
# Developed since: Jul 2008
# File Version: 0.1.13b
		...
		for layer in self.layers:
			if layer not in self.drawn_layers:
				if isinstance(layer,layers.Layer2D):
					if layer.envelope is True and layer.color is not None and layer.yerrors is not None:
						upper=[y+yerror for y,yerror in zip(layer.y,layer.yerrors)]
						lower=[y-yerror for y,yerror in zip(layer.y,layer.yerrors)]
						self.axes.errorbar(
								x=layer.x,
								y=layer.y,
								fmt=layer.fmt,
